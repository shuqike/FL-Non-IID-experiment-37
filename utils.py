...
def load_mnist_data(datadir):
    ...
    mnist_train_ds = MNIST_truncated(datadir, train=True, download=True, transform=transform, y_train='nmsl')
    mnist_test_ds = MNIST_truncated(datadir, train=False, download=True, transform=transform, y_train='nmsl')
...
def load_fmnist_data(datadir):
    ...
    mnist_train_ds = FashionMNIST_truncated(datadir, train=True, download=True, transform=transform, y_train='nmsl')
    mnist_test_ds = FashionMNIST_truncated(datadir, train=False, download=True, transform=transform, y_train='nmsl')
...
def load_svhn_data(datadir):
    ...
    svhn_train_ds = SVHN_custom(datadir, train=True, download=True, transform=transform, y_train='nmsl')
    svhn_test_ds = SVHN_custom(datadir, train=False, download=True, transform=transform, y_train='nmsl')
...
def load_cifar10_data(datadir):
    ...
    cifar10_train_ds = CIFAR10_truncated(datadir, train=True, download=True, transform=transform, y_train='nmsl')
    cifar10_test_ds = CIFAR10_truncated(datadir, train=False, download=True, transform=transform, y_train='nmsl')
...
def load_femnist_data(datadir):
    ...
    mnist_train_ds = FEMNIST(datadir, train=True, transform=transform, download=True, y_train='nmsl')
    mnist_test_ds = FEMNIST(datadir, train=False, transform=transform, download=True, y_train='nmsl')
...
def load_cifar100_data(datadir):
    ...
    cifar100_train_ds = CIFAR100_truncated(datadir, train=True, download=True, transform=transform, y_train='nmsl')
    cifar100_test_ds = CIFAR100_truncated(datadir, train=False, download=True, transform=transform, y_train='nmsl')
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4, annotation_skew_degree=None, dt_usage=1.0):
    #np.random.seed(2020)
    ...
    if partition == "homo":
    ...
    elif partition[:17] == "noniid-wronglabel":
        ...
        def label_map(lb_element):
            if partition == 'noniid-wronglabel-uniformrand':
                pool = list(range(K))
                pool.remove(lb_element)
                return np.random.choice(pool)
            elif partition == 'noniid-wronglabel-pairflip':
                return (lb_element + 1) % K
            else:
        ...
        for p in range(n_parties):
            for k in range(int(len(net_dataidx_map[p]) * annotation_skew_degree[p])):
                y_train[net_dataidx_map[p][k]] = label_map(y_train[net_dataidx_map[p][k]])
        # shuffle again
        for p in range(n_parties):
        ...
        for p in range(n_parties):
            ...
            for k in range(len(net_dataidx_map[p])):
                ...
                print('original label', out.data.shape, torch.max(out.data, 1))
    ...
    # control training data usage
    if dt_usage != 1.0 and dt_usage != '1' and dt_usage != '1.0':
        print('yes')
        if type(dt_usage) is str:
            print('yes1')
            dt_usage = dt_usage.split(' ')
            dt_usage = list(map(float, dt_usage))
            for i in range(n_parties):
                net_dataidx_map[i] = net_dataidx_map[i][ : int(len(net_dataidx_map[i]) * dt_usage[i])]
        else:
            print('yes2')
            for i in range(n_parties):
                net_dataidx_map[i] = net_dataidx_map[i][ : int(len(net_dataidx_map[i]) * dt_usage)]
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0, y_train=None):
    if dataset in ('mnist', 'femnist', 'fmnist', 'cifar10', 'svhn', 'generated', 'covtype', 'a9a', 'rcv1', 'SUSY', 'cifar100', 'tinyimagenet'):
        ...
        if dataset == "tinyimagenet":
            ...
            train_ds = dl_obj(datadir, dataidxs=dataidxs, train=True, transform=transform_train, download=True, y_train=y_train)
